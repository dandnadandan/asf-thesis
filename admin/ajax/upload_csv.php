...
try {
    ...
    foreach ($data as $index => $row) {
        try {
            switch ($uploadType) {
                case 'environmental':
                    $success = processEnvironmentalData($pdo, $row, $userId, $index + 1, $uploadId);
                    break;
                case 'outbreaks':
                    $success = processOutbreakData($pdo, $row, $userId, $index + 1, $uploadId);
                    break;
                case 'depopulation':
                    $success = processDepopulationData($pdo, $row, $userId, $index + 1, $uploadId);
                    break;
                case 'meat_movement':
                    $success = processMeatMovementData($pdo, $row, $userId, $index + 1, $uploadId);
                    break;
                default:
                    $success = false;
            }
 ...
 */
function processEnvironmentalData($pdo, $row, $userId, $rowNum, $uploadId = null) {
    ...
    $stmt = $pdo->prepare("INSERT INTO environmental_data
                          (location_name, latitude, longitude, province, city, barangay,
                           ...
                           atmospheric_pressure, recorded_by, recorded_at, upload_id)
                          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
 ...
 */
function processOutbreakData($pdo, $row, $userId, $rowNum, $uploadId = null) {
    ...
    $reportedDate = date(
        'Y-m-d H:i:s',
        strtotime($row['reported_date'])
    );
    
    $stmt = $pdo->prepare("
        INSERT INTO asf_outbreaks (
            outbreak_code,
            location_name,
            latitude,
            longitude,
    ...
    return $stmt->execute([
        ...
        trim($row['city']),
        $reportedDate,
        !empty($row['severity_level'])
            ? trim($row['severity_level'])
            : null,
        !empty($row['avg_temp'])
            ? floatval($row['avg_temp'])
            : null,
        $userId,
        $uploadId
    ]);
 ...
 */
function processDepopulationData($pdo, $row, $userId, $rowNum, $uploadId = null) {
    ...
    $stmt = $pdo->prepare("
        INSERT INTO depopulation_events (
            eventCode,
            location_name,
            latitude,
            longitude,
    ...
    return $stmt->execute([
        ...
        !empty($row['farm_name'])
            ...
            : null,
        $eventDate,
        !empty($row['head_count'])
            ? intval($row['head_count'])
            : 0,
        !empty($row['depopulation_method'])
            ? trim($row['depopulation_method'])
            : 'culling',
        $userId,
        $uploadId
    ]);
 ...
 */
function processMeatMovementData($pdo, $row, $userId, $rowNum, $uploadId = null) {
    $movementCode = 'MM' . date('Ymd') . rand(1000, 9999) . $rowNum;
    
    if (
        empty($row['source_location']) ||
        empty($row['source_city']) ||
        empty($row['destination_location']) ||
        empty($row['destination_city']) ||
    ...
    $movementDate = date(
        'Y-m-d',
        strtotime($row['movement_date'])
    );
    
    $stmt = $pdo->prepare("
        INSERT INTO meat_movement (
            movementCode,
            source_location,
            source_latitude,
            source_longitude,
    ...
    return $stmt->execute([
        ...
        !empty($row['source_latitude'])
            ? floatval($row['source_latitude'])
            : null,
        !empty($row['source_longitude'])
            ? floatval($row['source_longitude'])
            : null,
        $sourceProvince,
        trim($row['source_city']),
        trim($row['destination_location']),
        !empty($row['destination_latitude'])
            ? floatval($row['destination_latitude'])
            : null,
        !empty($row['destination_longitude'])
            ? floatval($row['destination_longitude'])
            : null,
        $destProvince,
        ...
        !empty($row['quantity_kg'])
            ? floatval($row['quantity_kg'])
            : null,
        !empty($row['quantity_heads'])
            ? intval($row['quantity_heads'])
            : null,
        $userId,
        $uploadId
    ]);
